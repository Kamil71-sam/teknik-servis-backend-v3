...
const guvenlik = require('./guvenlik'); // MÜDÜR: Şefi içeri aldık
...
// --- 3. ANA ŞALTERLER (GÜVENLİK DUVARI) ---

// 🔓 AÇIK KAPI: Giriş yapmak için Token gerekmez
app.use("/auth", authRoutes); 

// 🔐 KİLİTLİ KAPILAR: Sadece Yaka Kartı (Token) olanlar girebilir
// Müdür: app.use içine 'guvenlik' ekleyerek her odaya bekçi diktik.
app.use("/customers", guvenlik, customersRoutes);
app.use("/api/customers", guvenlik, customersRoutes);
app.use("/devices", guvenlik, devicesRoutes);
app.use("/services", guvenlik, servicesRoutes);
app.use("/service-notes", guvenlik, serviceNotesRoutes);
app.use("/uzman", guvenlik, uzmanRoutes);
app.use("/api/appointments", guvenlik, appointmentRoutes);
app.use("/api/firm", guvenlik, firmRoute);
app.use('/api/operation', guvenlik, operationRoutes);
app.use('/api/stok', guvenlik, stokRoutes);
app.use('/api/kasa', guvenlik, kasaRoutes);
app.use('/api/tahsilat', guvenlik, tahsilatRoutes);
app.use('/api/kasa_v2', guvenlik, kasaV2Router);
...
// Gümrük Kapısı (Bunu da güvenliğe aldık ki sinsi sızmalar olmasın)
app.post('/api/operation/tahsilat-kaydet', guvenlik, async (req, res) => {
    const { id, usta_maliyet, tahsil_edilen_tutar } = req.body;
    try {
        const query = `UPDATE appointments SET usta_maliyet = $1, tahsil_edilen_tutar = $2, status = 'Mali Onay Bekliyor' WHERE id = $3`;
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`📡 Yerel Adres: http://localhost:${PORT}`);
  console.log(`📱 Telefon İçin: http://192.168.1.50:${PORT}`);
