...
router.post("/dashboard", async (req, res) => {
    ...
    try {
        // 1. SERVİSLERİ SAYAN BÖLÜM
        const statsRes = await db.query(
            `SELECT 
                COUNT(*) FILTER (WHERE status NOT IN ('Pasif', 'Teslim Edildi', 'İptal Edildi', 'İptal')) as atanan,
                COUNT(*) FILTER (WHERE status IN ('Onaylandı', 'Tamirde', 'İşlem Bekliyor')) as aktif,
        ...
        // 2. RANDEVULARI SAYAN BÖLÜM (KUTU SAYACI - ESNEK ARAMA)
        const randevuRes = await db.query(
            `SELECT COUNT(*) as toplam_randevu 
             FROM appointments 
             WHERE TRIM(assigned_usta) ILIKE $1 
             AND status IN ('Beklemede', 'Devam Ediyor', 'İşlem Bekliyor')`, 
            [`%${ustaParam}%`]
        );

        // 3. ALTTAKİ SON İŞLER LİSTESİ BÖLÜMÜ
        ...
        // MÜDÜR: EKRANA VERİLERİN GÖNDERİLDİĞİ YER
        res.json({
            success: true,
            data: {
                ...
                // RANDEVU KUTUSUNA SAYI BURADAN GİDİYOR:
                randevuSayisi: parseInt(randevuRes.rows[0].toplam_randevu) || 0, 
                
                sonIsler: sonIslerRes.rows,
            }
        });
    } catch (err) {
        // Eğer sistem çökerse terminalde kırmızı kırmızı hatayı görelim:
        console.error("🚨 DASHBOARD ÇÖKTÜ:", err.message);
